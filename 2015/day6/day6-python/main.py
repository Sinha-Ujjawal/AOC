...
def solve(
    turn_on_update_fn: Callable[[int], int],
    turn_off_update_fn: Callable[[int], int],
    toggle_update_fn: Callable[[int], int],
    ...
    def update_grid(box: Box, update_fn: Callable[[int], int]) -> None:
        ...
        for row in range(xl, xu + 1):
            for col in range(yl, yu + 1):
                idx = (row * GRID_SIZE) + col
                grid[idx] = max(0, update_fn(grid[idx]))
...
def solve_part1(instructions: Iterable[Instruction]) -> int:
    ...
    return sum(1 for val in final_grid if val > 0)
...
def solve_part2(instructions: Iterable[Instruction]) -> int:
    ...
    return sum(val for val in final_grid if val > 0)
