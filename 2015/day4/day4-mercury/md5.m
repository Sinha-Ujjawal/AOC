% Stole from https://raw.githubusercontent.com/Mercury-Language/mercury/9656e13e73cfb5269834a7c6f8b4f17c02f7daba/compiler/md5.m %
%---------------------------------------------------------------------------%
...
static void MD5_Final(unsigned char *result, MD5_CTX *ctx)
{
    ...
    ctx->lo <<= 3;
    MC_OUT(&ctx->buffer[56], ctx->lo)
    MC_OUT(&ctx->buffer[60], ctx->hi)

    body(ctx, ctx->buffer, 64);

    MC_OUT(&result[0], ctx->a)
    MC_OUT(&result[4], ctx->b)
    MC_OUT(&result[8], ctx->c)
    MC_OUT(&result[12], ctx->d)
